...
namespace std{
	...
	%extend 
	{
		...
		%apply void *VOID_INT_PTR{ std::list< T >::iterator *create_iterator_begin }
		%apply void *VOID_INT_PTR{ std::list< T >::iterator *swigiterator }

		std::list< T >::iterator *create_iterator_begin() {
			return new std::list< T >::iterator($self->begin());
		}

		bool is_iterator_good(std::list< T >::iterator *swigiterator) {
			if (*swigiterator == self->end())
				return false;
			return true;
		}

		const value_type& get_next_key(std::list< T >::iterator *swigiterator) {
			std::list< T >::iterator iter = *swigiterator;
			(*swigiterator)++;
			return *iter;
		}

		void destroy_iterator(std::list< T >::iterator *swigiterator) {
			delete swigiterator;
		}
